...
from lscaleuq.lib.boolean_flags import boolean_flags
...
class PostFixColumn(ProgressColumn):
    ...
    def render(self, task: "Task") -> Text:
        ...
        if _postfix:
            ...
            _text = (
                "• "
                + ", ".join(key + "=" + postfix[key].strip() for key in postfix.keys())
                + " ]"
            )
        else:
            _text = "]"
...
def get_progress(
    ...
    disable = (
        disable
        or boolean_flags(os.getenv("TQDM_DISABLE", False))
        or boolean_flags(os.getenv("RICH_DISABLE", False))
        or boolean_flags(os.getenv("DEBUG", False))
    )
